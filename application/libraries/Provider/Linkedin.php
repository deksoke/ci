...
class OAuth2_Provider_Linkedin extends OAuth2_Provider {
  ...
  public function get_user_info(OAuth2_Token_Access $token) {
    ...
    $url_profile = 'https://apis.linkedin.com/v1/people/~?format=json&' . http_build_query(array(
                'oauth2_access_token' => $token->access_token,
    ));
    $user = json_decode(file_get_contents($url_profile), true);

    $url_email = 'https://apis.linkedin.com/v1/people/~/email-address?format=json&' . http_build_query(array(
                'oauth2_access_token' => $token->access_token,
    ));
